...
const CoinModel = React.forwardRef<THREE.Object3D, CoinModelProps>(
  ({ scale, side, rotate, orbit }, ref) => {
...
const CoinLogic = ({
  ...
  coinRef: React.RefObject<THREE.Object3D>;
}) => {
  ...
  useFrame(() => {
    if (coinRef.current) {
      const forward = new THREE.Vector3(0, 0, 1).applyQuaternion(
        coinRef.current.quaternion,
      );

      // куда смотреть от монеты что бы увидеть камеру
      const toCamera = new THREE.Vector3()
        .subVectors(camera.position, coinRef.current.position)
        .normalize();
